...
const ExperienceCarouselItem = ({
  ...
  return (<div className={`w-[272px] h-full ${ backgroundColor ? `bg-${backgroundColor}` : 'bg-gohan' } rounded-lg flex flex-col px-5 pt-5 pb-6 sm:my-8 sm:h-[320px] ${index && index % 2 ? 'sm:mt-[40%]' : ''}`}>
    <div className="flex items-center justify-center pt-3 pb-4">
      <Image src={icon} alt={title} />
    </div>
    <p className="text-moon-18 text-center font-semibold mb-4 w-full px-4">
      {title}
    </p>
